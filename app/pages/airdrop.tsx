...
const CallToActionButton = ({
	...
	if (!verified) {
		return <Button title="Verify your account" onClick={onOpenVerification} />;
	}
	...
	const renderer = ({
		...
		return (
			<>
				...
				<div className="my-4">
					<CallToActionButton address={address} verified={verified} onOpenVerification={onOpenVerification} />
				</div>
			</>
		);
	};
...
const AirdropPage = () => {
	...
	return (
		<>
				...
				<div className="p-6 md:mt-8 md:px-16 flex flex-col md:flex-row md:space-x-16 text-zinc-800 relative text-center md:text-left">
					<div className="w-full md:w-1/2 flex flex-col">
						...
						<span className="text-base text-zinc-800">
							We&apos;re running a retroactive airdrop campaign with a monthly rewards pool. If you trade
							on OpenPeer, you will be rewarded based on your volume and providing liquidity to the
							protocol.
					...
					<div className="w-full mt-8 md:mt-0 md:w-1/2 rounded-xl mx-auto p-[4px] bg-gradient-to-r from-[#3C9AAA] to-[#2A4BE3]">
						<AirdropCountdown
							...
							verified={verified}
							tokens={tokens}
							onOpenVerification={() => setOpenVerification(true)}
						/>
					</div>
				</div>
			...
			<QuadrataClient
				onFinish={() => setOpenVerification(false)}
				open={openVerification}
				onHide={() => setOpenVerification(false)}
			/>
		</>
	);
};
