...
const OrderPaymentMethod = ({ order, updateOrder }: BuyStepProps) => {
	...
	const banks = type === 'SellList' ? list.payment_methods.map((pm) => ({ ...pm.bank, id: pm.id })) : list.banks;
					...
					snakecaseKeys(
						{
							order: {
								...
								price: order.price,
								paymentMethod: type === 'BuyList' ? paymentMethod : { id: bank?.id }
							},
							data,
							address,
				...
				<>
					...
					{!id || edit ? (
						<div className="mb-2">
							...
							<div>
								{schema.map(
									({ id: schemaId, label, placeholder, type: schemaType = 'text', required }) => {
